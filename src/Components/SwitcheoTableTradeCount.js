...
const rows = [
  ...
  { id: 'trades', numeric: true, disablePadding: false, label: 'Number of Trades' },
];
    ...
    <Toolbar>
      <div className={classes.title}>
        <Typography id="tableTitle" variant='title'>
          Switcheo NEO Trade Count (Pending Review)
        </Typography>
      </div>
    </Toolbar>
  );
...
class TradeCountTable extends Component {
  state = {
    order: 'desc',
    orderBy: 'total',
    selected: [],
  ...
  getSwitcheoData = async () => {
    try {
      const res = await axios.get(this.props.api + '/switcheo/addresses/trades/count');
...
TradeCountTable.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(TradeCountTable);
