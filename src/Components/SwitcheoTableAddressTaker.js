...
const rows = [
  ...
  { id: 'takes', numeric: true, disablePadding: false, label: 'Number of Taker Orders' },
];
...
class AddressTakerTable extends Component {
  state = {
    order: 'desc',
    orderBy: 'total',
    selected: [],
  ...
  getSwitcheoData = async () => {
    try {
      const res = await axios.get(this.props.api + '/switcheo/addresses/takes');
...
AddressTakerTable.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(AddressTakerTable);
